...
class TTSRequest(BaseModel):
    text: str
    voice: str                  # preset name or base64 data URL of reference audio
...
class VoiceUploadRequest(BaseModel):
    name: str
    voice: str                  # base64 data URL of reference audio


def _sanitize_voice_name(name: str) -> str:
    cleaned = re.sub(r"[^\w\-]+", "_", name.strip())[:40]
    if not cleaned:
        raise HTTPException(status_code=400, detail="invalid voice name")
    return cleaned


def _convert_file_to_wav(path: Path) -> str:
    tmp_out = tempfile.NamedTemporaryFile(suffix=".wav", delete=False)
    tmp_out.close()
    result = subprocess.run(
        ["ffmpeg", "-y", "-i", str(path), "-ar", "24000", "-ac", "1", tmp_out.name, "-loglevel", "error"],
        capture_output=True,
    )
...
def _resolve_voice_path(voice: str) -> tuple[str, bool]:
    """Return (audio path, should_delete). Preset names resolve under VOICES_DIR."""
    if voice.startswith("data:"):
        return _decode_voice(voice), True
    ...
    for ext in (".wav", ".mp3", ".webm", ".ogg", ".m4a"):
        ...
        if path.is_file():
            ...
            return _convert_file_to_wav(path), True

    raise HTTPException(status_code=404, detail=f"voice not found: {name}")


# Voice embedding cache: md5(voice_key:exaggeration) → tts.conds
_CONDS_CACHE: dict[str, object] = {}
_CONDS_CACHE_MAX = 4

def _prepare_conditionals_cached(voice: str, exaggeration: float) -> None:
    key = hashlib.md5(f"{voice}:{exaggeration:.3f}".encode()).hexdigest()
    if key in _CONDS_CACHE:
        tts.conds = _CONDS_CACHE[key]
        return
    ref_path, delete_after = _resolve_voice_path(voice)
    try:
...
@app.post("/api/voices")
async def upload_voice(req: VoiceUploadRequest):
    _require_ready()
    VOICES_DIR.mkdir(parents=True, exist_ok=True)
    name = _sanitize_voice_name(req.name)
    ref_path, delete_after = _resolve_voice_path(req.voice)
    try:
        out_path = VOICES_DIR / f"{name}.wav"
        if ref_path.endswith(".wav") and not delete_after:
            import shutil
            shutil.copy2(ref_path, out_path)
        else:
            result = subprocess.run(
                ["ffmpeg", "-y", "-i", ref_path, "-ar", "24000", "-ac", "1", str(out_path), "-loglevel", "error"],
                capture_output=True,
            )
            if result.returncode != 0:
                raise HTTPException(status_code=400, detail="could not convert voice audio")
    finally:
        if delete_after:
            os.unlink(ref_path)
    return {"name": name, "ext": "wav"}
